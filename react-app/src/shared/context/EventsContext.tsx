...
import type { Ticket, EventsState, EventsContextType, Event } from "./Interfaces";
...
// 🔹 Mapea evento del backend a Ticket (para listas)
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (alive) {
          const featuredTickets = (featuredRes.data?.data ?? []).map(mapApiEventToTicket);
          dispatch({ type: "SET_FEATURED_TICKETS", payload: { tickets: featuredTickets } });

          const approvedTickets = (approvedRes.data?.data ?? []).map(mapApiEventToTicket);
